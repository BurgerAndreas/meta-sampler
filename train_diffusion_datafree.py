...
# ---------------------------
# 1. Define our toy function and its energy.
# ---------------------------
def f(x):
    # f(x) = (x**2 - 1)**3
    return (x**2 - 1)**3
...
# ---------------------------
# 5. Iterative Diffusion Energy Matching: Training without target Boltzmann samples.
# ---------------------------
...
batch_size = 128
lambda_energy = 0.1       # weight for the energy penalty

n_samples = 10000  # number of pseudo-data points

# Initialize pseudo-data: sample uniformly from [-3, 3]
with torch.no_grad():
    samples_buffer = torch.FloatTensor(n_samples, 1).uniform_(-3, 3).to(device)

print("Starting iterative diffusion energy matching training...")
for outer in range(outer_iterations):
    ...
    for epoch in range(inner_epochs):
        ...
        for i in range(n_batches):
            ...
            t_norm = (t.float() / T).unsqueeze(1)  # normalized timestep, shape (batch,1)
            ...
            # Energy loss: encourage x0_pred to have low energy
            loss_energy = energy(x0_pred).mean()
        ...
        if (epoch + 1) % 100 == 0:
            print(f"Outer iter {outer+1}/{outer_iterations}, Epoch [{epoch+1}/{inner_epochs}] Loss: {epoch_loss/n_batches:.5f}")
    ...
    with torch.no_grad():
        ...
        for t_inv in range(T-1, -1, -1):
            t_batch = (torch.ones(n_samples, 1) * (t_inv / T)).to(device)  # normalized time
            beta_t = betas[t_inv]
    ...
    # Optionally: print average energy of the current pseudo-data.
    avg_energy = energy(samples_buffer).mean().item()
    print(f"After outer iteration {outer+1}, average energy of pseudo-data: {avg_energy:.5f}")
...
# Compute energy on grid (using numpy version of f_prime)
...
ax.hist(final_samples, bins=50, density=True, alpha=0.6, label="Generated Samples (iDEM)")
ax.plot(x_grid, p_true, 'r-', lw=2, label="Target Boltzmann Distribution")
ax.set_title("Iterative Diffusion Energy Matching for Sampling\nfrom the Boltzmann Distribution")
ax.set_xlabel("x")
