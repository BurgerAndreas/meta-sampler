...
def test_ic_transform_energy_consistency():
    ...
    simulation = app.Simulation(alanine_system.topology, alanine_system.system, integrator)

    # ----------------------------------------------------------
    # Setup a second simulation with the pdb file
    # ----------------------------------------------------------
    
    pdb = openmm.app.PDBFile(os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "alanine-dipeptide-nowater.pdb"))
    ...
    simulation_bgmol = app.Simulation(system_bgmol.topology, system_bgmol.system, integrator_bgmol)
    ...
    for idx in random_indices:
        ...
        energy_calculated = state.getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole)
        ...
        energy_calculated_pdb = _state.getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole)
        ...
        energy_calculated_bgmol = state_bgmol.getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole)
        ...
        print(f"Energy difference: {np.abs(energy_calculated - energy_dataset):.3f} kJ/mol")
        print(f"Energy difference pdb: {np.abs(energy_calculated_pdb - energy_calculated):.3f} kJ/mol ({energy_calculated:.1f})")
        print(f"Energy difference bgmol: {np.abs(energy_calculated_bgmol - energy_calculated):.3f} kJ/mol ({energy_calculated:.1f})")
