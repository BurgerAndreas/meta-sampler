...
def test_dihedral_functions():
    ...
    # Run NumPy version.
    positions_np_modified = set_dihedral(positions_np.copy(), indices, target_angle, atoms_to_rotate)
    
    # Run Torch version.
    positions_torch_modified = set_dihedral_torch(positions_torch, indices, torch.tensor(target_angle, dtype=torch.float64), atoms_to_rotate)
    
    # Convert Torch result to NumPy for comparison.
    positions_torch_np = positions_torch_modified.detach().numpy()
    
    # Compare the two results.
    assert np.allclose(positions_np_modified, positions_torch_np, atol=1e-6), "Mismatch between NumPy and Torch results"
...
if __name__ == '__main__':
    ...
    positions = torch.tensor(np.random.rand(10, 3), dtype=torch.float64, requires_grad=True)
    indices = (1, 2, 3, 4)
    target_angle = torch.tensor(math.pi/3.0, dtype=torch.float64)
    atoms_to_rotate = "psi"
    positions_new = set_dihedral_torch(positions, indices, target_angle, atoms_to_rotate)
