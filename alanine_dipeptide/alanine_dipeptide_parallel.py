...
from simtk.openmm.app import PDBFile, ForceField, Simulation, NoCutoff
from simtk.openmm import VerletIntegrator, Platform
from simtk.unit import nanometer, kilojoule_per_mole, picoseconds, Quantity

# ---------------------------------------------------------------------------
# Helper functions for rotating dihedrals (same as before)
# ---------------------------------------------------------------------------
...
def compute_dihedral(p0, p1, p2, p3):
    ...
    # Normalize b1 so that it does not influence the magnitude.
...
def set_dihedral(positions, indices, target_angle):
    ...
    > **Note:** This is a simplified routine. In practice, you would determine the set of atoms
    > to rotate based on molecular connectivity.
    """
    i, j, k, l = indices
    ...
    # For simplicity, assume that all atoms with index >= l are rotated.
...
def build_alanine_dipeptide(phi, psi):
    ...
    Loads a template PDB file and then rotates the appropriate groups of atoms.
    ...
    pdb_filename = 'alanine_dipeptide_template.pdb'
    if not os.path.exists(pdb_filename):
        raise FileNotFoundError(f"{pdb_filename} not found. Please ensure the file is available.")

    pdb = PDBFile(pdb_filename)
    positions = np.array(pdb.positions.value_in_unit(nanometer))
    
    # Example indices for φ and ψ (adjust these to match your template's atom ordering).
    phi_indices = [1, 2, 3, 4]
    psi_indices = [2, 3, 4, 5]
    ...
    return Quantity(positions, nanometer)

def compute_energy_and_forces(dihedrals):
    """
    Given a tensor dihedrals of shape [2] (with dihedrals[0]=φ and dihedrals[1]=ψ, in radians),
    build an alanine dipeptide configuration, evaluate its energy and forces using the force field,
    and return these as PyTorch tensors.
    
    Parameters:
      dihedrals: Torch tensor of shape [2] containing the dihedral angles (in radians)
    ...
    """
    # Convert tensor entries to float values.
    phi = float(dihedrals[0].item()) if hasattr(dihedrals[0], 'item') else float(dihedrals[0])
    psi = float(dihedrals[1].item()) if hasattr(dihedrals[1], 'item') else float(dihedrals[1])
    
    # Build configuration.
    positions = build_alanine_dipeptide(phi, psi)
    
    # Load topology from the template.
    pdb = PDBFile('alanine_dipeptide_template.pdb')
    
    # Create the force field.
    forcefield = ForceField('amber99sb.xml')
    
    # Create the system (using NoCutoff for simplicity).
    system = forcefield.createSystem(pdb.topology, nonbondedMethod=NoCutoff)
    
    # Set up the simulation. Using the Reference platform for compatibility.
    ...
    # Compute state: energy and forces.
