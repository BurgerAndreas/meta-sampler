...
class PainnMessage(nn.Module):
    ...
    def forward(self, node_scalar, node_vector, edge, edge_diff, edge_dist):
        ...
        assert not torch.isnan(filter_weight).any(), f"filter_weight: \n{filter_weight}"
        assert not torch.isnan(scalar_out).any(), f"scalar_out: \n{scalar_out}\nnode_scalar: \n{node_scalar}"
        assert not torch.isnan(filter_out).any(), f"filter_out: \n{filter_out}"
        ...
        assert not torch.isnan(message_vector).any(), f"message_vector: \n{message_vector}"
...
class PainnUpdate(nn.Module):
    ...
    def forward(self, node_scalar, node_vector):
        ...
        node_scalar: [n_atoms_batch, node_size]
        node_vector: [n_atoms_batch, 3, node_size]
        """
        Uv = self.update_U(node_vector)
        Vv = self.update_V(node_vector)
        assert not torch.isnan(Uv).any(), f"Uv: \n{Uv}\nnode_vector: \n{node_vector}"
        assert not torch.isnan(Vv).any(), f"Vv: \n{Vv}\nnode_vector: \n{node_vector}"
        
        Vv_norm = torch.linalg.norm(Vv, dim=1)
        mlp_input = torch.cat((Vv_norm, node_scalar), dim=1)
        mlp_output = self.update_mlp(mlp_input)
        assert not torch.isnan(mlp_output).any(), f"mlp_output: \n{mlp_output}"
        ...
        assert not torch.isnan(delta_s).any(), f"delta_s: \n{delta_s}"
        assert not torch.isnan(delta_v).any(), f"delta_v: \n{delta_v}"
...
class PainnModel(nn.Module):
    ...
    def forward(self, num_atoms, elems, cell, coord, pairs, n_diff, num_pairs, compute_forces=True):
        """
        Batch contains
        - num_atoms: number of atoms in the batch[B] 
        - elems: atom type Z [n_atoms_batch]. n_atoms_batch = sum(num_atoms) 
        - cell: cell [B*3, 3] 
        - coord: atom positions [n_atoms_batch, 3] # predict
        - pairs: pairs of atoms [n_pairs_batch, 2]. n_pairs_batch = sum(num_pairs) 
        - n_diff: difference in positions between pairs [n_pairs_batch, 3] 
        - num_pairs: number of pairs in the batch [B] 
        Outputs:
        - energy: potential energy [B] 
        - forces: forces [n_atoms_batch, 3] 
        """
        ...
        for message_layer, update_layer in zip(self.message_layers, self.update_layers):
            ...
            _l += 1
            # TODO: nan here
            assert not torch.isnan(node_scalar).any(), f"node_scalar after {_l} / {len(self.message_layers)} layers: \n{node_scalar}"
            assert not torch.isnan(node_vector).any(), f"node_vector after {_l} / {len(self.message_layers)} layers: \n{node_vector}"
            assert torch.isfinite(node_scalar).all(), f"node_scalar after {_l} / {len(self.message_layers)} layers: \n{node_scalar}"
            assert torch.isfinite(node_vector).all(), f"node_vector after {_l} / {len(self.message_layers)} layers: \n{node_vector}"
        ...
        assert not torch.isnan(image_idx).any(), f"image_idx: \n{image_idx}"
        
        energy = torch.zeros_like(num_atoms).float()        
        energy.index_add_(0, image_idx, node_scalar)
        assert not torch.isnan(energy).any(), f"energy: \n{energy}"
        ...
        if compute_forces:
            ...
            i_forces = torch.zeros_like(coord).index_add(0, edge[:, 0], dE_ddiff)
            j_forces = torch.zeros_like(coord).index_add(0, edge[:, 1], -dE_ddiff)
...
class PainnModel_predict(nn.Module):
    ...
    def forward(self, num_atoms, elems, cell, coord, pairs, n_diff, num_pairs, compute_forces=True):
        ...
        for message_layer, update_layer in zip(self.message_layers, self.update_layers):
            node_scalar, node_vector = message_layer(node_scalar, node_vecstor, edge, edge_diff, edge_dist)
            node_scalar, node_vector = update_layer(node_scalar, node_vector)
