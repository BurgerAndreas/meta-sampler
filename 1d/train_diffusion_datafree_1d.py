...
if __name__ == "__main__":
    ...
    grad_f = torch.autograd.grad(pes_val, xvals, grad_outputs=torch.ones_like(pes_val), create_graph=True)[0]
    ...
    print("diff", (np.abs(grad_f_np) - np.linalg.norm(grad_f_np[:, None], axis=1)).mean())
    ...
    print("Saved figure to ", plotting_dir / "diffusion_datafree_1d_target_distribution.png")
    ...
    for outer in range(outer_iterations):
        ...
        print(f"After outer iteration {outer+1}, average pseudo_energy of pseudo-data: {avg_pseudo_energy:.5f}")
    ...
    ax.set_title("Iterative Diffusion pseudo_energy Matching for Sampling\nfrom the Boltzmann Distribution")
    ax.set_xlabel("x")
