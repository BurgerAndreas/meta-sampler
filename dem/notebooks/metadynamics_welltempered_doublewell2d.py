...
def test_energy_and_forces(energy_fn):
    ...
    test_points = torch.cat([energy_fn.minima_points, energy_fn.transition_points], dim=0)
...
def plot_energy_landscape(energy_fn, resolution=100, bounds=(-3, 3)):
    ...
    plt.scatter(energy_fn.minima_points[:, 0], energy_fn.minima_points[:, 1], color='red', s=100, marker='o', label='Minima')
    plt.scatter(energy_fn.transition_points[:, 0], energy_fn.transition_points[:, 1], color='white', s=100, marker='x', label='Transition State')
...
def run_metadynamics_2d(energy_fn):
    ...
    for step in tqdm(range(n_steps)):
        ...
        if step % deposit_interval == 0:
            ...
            effective_hill_height = initial_hill_height * torch.exp(-current_bias / delta_T)
    ...
    # Plot the trajectory
    plt.subplot(1, 3, 1)
    plt.plot(trajectory[:, 0].numpy(), trajectory[:, 1].numpy(), 'b-', alpha=0.6, linewidth=0.8)
    plt.scatter(trajectory[0, 0].numpy(), trajectory[0, 1].numpy(), color='green', s=100, label='Start')
    plt.scatter(trajectory[-1, 0].numpy(), trajectory[-1, 1].numpy(), color='red', s=100, label='End')
    plt.xlabel('x')
    plt.ylabel('y')
    ...
    contour_orig = plt.contourf(X.numpy(), Y.numpy(), original_energy_grid.numpy(), 50, cmap='viridis')
    ...
    if peaks is not None and len(peaks) > 0:
        plt.scatter(transition_states[:, 0].numpy(), transition_states[:, 1].numpy(), color='white', s=100, marker='x', label='Transition State')
    ...
    for i, point in enumerate(grid_points):
        bias_energies[i] = bias_potential(point, hill_positions, hill_heights, hill_width)
    ...
    contour_biased = plt.contourf(X.numpy(), Y.numpy(), total_energy_grid.numpy(), 50, cmap='viridis')
    ...
    if peaks is not None and len(peaks) > 0:
        ...
        plt.scatter(transition_times, transition_energies, color='red', s=80, marker='o', 
