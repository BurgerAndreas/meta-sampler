...
# os.environ['DISPLAY'] = ''  # Set empty display

"""
Trains a PaiNN (Polarizable Atomic Interaction Neural Network) for molecular simulations.

Paper:
training for up to 1 000 000 steps
0.0001 initial learning rate
exponential decay learning rate scheduler with a coefficient of 0.96 is used to adjust the learning
rate for every 100 000 learning steps. 
The dataset is split into
a training set (90%) and a validation set (10%)
the vali-
dation set is used for early stopping when the error of forces is
small enough. 
cutoff radii are all set to 5 Å
batchsize 16
force weight is set to 0.99

Architecture that worked best:
128 nodes, 4 layers, ? steps
"""
...
def get_arguments(arg_list=None):
    ...
    parser.add_argument(
        "--load_model",
        type=str,
        help="Load model parameters from previous run (optional)",
    )
    parser.add_argument(
        "--cutoff",
        type=float,
        help="Atomic interaction cutoff distance [Angstrom]",
        default=5.0,
    )
    parser.add_argument(
        "--split_file",
        type=str,
        help="Train/test/validation split file json (optional)",
    )
    parser.add_argument(
        "--val_ratio",
        type=float,
    ...
    parser.add_argument(
        "--node_size", type=int, help="Size of hidden node states", default=96
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        help="Path to output directory",
        default="andreas_models",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cfg",
        type=str,
        help="Path to config file (optional). e.g. 'arguments.toml'"
    )
...
def main():
    ...
    print(f"-"*40)
    print("Example batch of the dataset:")
    batch_host = next(iter(train_loader))
    batch = {
    ...
    num_pairs = torch.sum(torch.tensor([n * (n - 1) / 2 for n in batch["num_atoms"]]))
    print("")
    print(f"num_pairs: {num_pairs} vs {torch.sum(batch['num_pairs'])} vs {batch['pairs'].shape[0]}")
    print(f"-"*40)
    ...
    for epoch in itertools.count():
        for batch_host in tqdm(train_loader):
            ...
            if (step % args.log_interval == 0) or ((step + 1) == args.max_steps):
                ...
                #     )
                # )
                tqdm.write(f"step={step}: {eval_formatted}")
                training_time = 0
