...
"""
Runs accelerated MD simulations.
Implements Langevin dynamics (temperature-controlled molecular dynamics) 
using a regular ML calculator or PLUMED (for metadynamics)
Includes uncertainty quantification from an ensemble of models
Uses ASE (Atomic Simulation Environment) for the MD.
"""
...
def get_arguments(arg_list=None):
    ...
    parser.add_argument(
        "--plumed",
        action="store_true",
        help="Use plumed for MD, which uses metadynamics. Alternatively, use the default calculator.",
    )
    parser.add_argument(
        "--plumed_restart",
        action="store_true",
        help="Restart metadynamics or not. Default is False.",
    )
    parser.add_argument(
        "--random_seed",
        type=int,
