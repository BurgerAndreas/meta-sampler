...
class GMMPseudoEnergy(BaseEnergyFunction):
    ...
    def __init__(
        ...
        **gmm_kwargs
        ...
        self.gmm_potential = GMM(dimensionality=dimensionality, device=device, **gmm_kwargs)
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pseudo_energy = self.energy_weight * energy + self.force_weight * force_magnitude
    ...
    def log_on_epoch_end(self, *args, **kwargs):
        ...
        if kwargs.get('wandb_logger') is not None and kwargs.get('latest_samples') is not None:
            ...
            if self.curr_epoch % self.gmm_potential.plot_samples_epoch_period == 0:
                ...
                grid_points = torch.tensor(np.stack([X.flatten(), Y.flatten()], axis=1), dtype=torch.float32, device=self.device)
                ...
                wandb_logger.log_image(f"{prefix}pseudo_energy_surface", [fig_to_image(fig)])
                plt.close()
    ...
    def validate_transition_states(self, transition_points, abs_ev_tol=1e-6, grad_tol=1e-1):
        """
    ...
    def find_saddle_points_scipy(self, grid_size=200, bounds=(-56, 56)):
        ...
        for point in tqdm(candidate_points, total=len(candidate_points)):
            ...
            x = torch.tensor(result.x, dtype=torch.float32, requires_grad=True, device=self.device)
            
            # Compute Hessian and check eigenvalues
            hessian = torch.func.hessian(lambda x: -self.gmm_potential.gmm.log_prob(x))(x)
...
def grad_neg_log_prob(x_np, gmm):
    ...
    if not isinstance(x_np, torch.Tensor):
        x_torch = torch.tensor(x_np, dtype=torch.float32, requires_grad=True, device=gmm.device)
    else:
        x_torch = x_np
