...
for epoch in range(num_epochs):
    ...
    for i in range(n_batches):
        ...
        x0 = train_samples[indices].to(
            device
        )  # clean sample from the Boltzmann distribution, shape: (batch, 1)
...
for t_inv in range(T - 1, -1, -1):
    t_batch = (torch.ones(n_gen, 1) * (t_inv / T)).to(
        device
    )  # normalized t (scalar repeated for batch)
